...
const currentDoctor = computed(() => {
    return page.props.auth.user.nombre_usuario || 'Dr. Roberto Méndez';
});
...
const form = useForm({
    ...
    tipo_sangre: props.patient.tipo_sangre || '',
    alergias: props.patient.alergias || '',
    observaciones: props.patient.observaciones || '',
    contacto_emergencia_nombre: props.patient.contacto_emergencia_nombre || '',
    contacto_emergencia_parentesco: props.patient.contacto_emergencia_parentesco || '',
    contacto_emergencia_telefono: props.patient.contacto_emergencia_telefono || '',
});

const submit = () => {
    form.put(route('doctor.patients.update', props.patient.id_paciente));
};
</script>
...
<template>
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between w-full">
                ...
                <div class="flex items-center gap-2 text-sm text-slate-500 font-semibold">
                    ...
                    <Link :href="route('doctor.patients.show', patient.id_paciente)" class="hover:text-blue-600 transition">Ficha #PAC-{{ String(patient.id_paciente).padStart(4, '0') }}</Link>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                    </svg>
                    <span class="text-slate-800">Modificar</span>
                </div>
        ...
        <Head title="Modificar Expediente - ProDoctor" />
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 p-8 shadow-sm space-y-8">
                        <div>
                            ...
                            <p class="text-xs text-slate-450 mt-1">Modifica la información básica del expediente médico.</p>
                        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 p-6 flex flex-col items-center shadow-sm">
                        <!-- Photo Box -->
                        <div class="w-28 h-28 rounded-full border-4 border-slate-100 bg-slate-50 flex items-center justify-center text-slate-450 relative overflow-hidden shadow-inner group cursor-pointer mb-4">
                            <div class="absolute inset-0 bg-blue-50/20 border border-blue-200 flex items-center justify-center font-black text-2xl text-blue-700">
                                {{ (patient.nombres[0] + patient.apellidos[0]).toUpperCase() }}
                            </div>
                        </div>
                        <span class="text-xs font-semibold text-slate-400 uppercase tracking-wide">FOTO DEL PACIENTE</span>
                        <span class="text-[10px] text-slate-400 mt-1">Modificar imagen de expediente</span>
